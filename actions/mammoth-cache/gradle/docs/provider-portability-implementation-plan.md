...
- temporary portability compatibility aliases/wrappers have been removed

The shared portability prep is complete. The remaining work is provider-specific validation and implementation.
...
- Migration targets:
  ...
  - `src/entrypoints/cli/prepare.ts`: output/reporter/state
  - `src/entrypoints/cli/finalize.ts`: reporter/state
...
- Current state:
  - runtime-host capabilities live under `src/runtime-host/types.ts`
  - provider implementations can still use `CompositeRuntimeHost` as a convenience type
  - shared flows and entrypoints depend on narrower runtime capabilities instead of a monolithic GitHub-named host alias
...
- Current state:
  ...
  - GitHub provider entrypoints call the shared prepare/finalize entrypoints directly
...
- Current state:
  ...
  - temporary compatibility aliases around runtime/storage/provider naming have been removed
...
- Current state:
  - provider-neutral prepare/finalize entrypoints now live under `src/entrypoints/cli/**`
  - GitHub entrypoints under `src/ci/github/*.ts` invoke those shared entrypoints directly
...
The current codebase now satisfies those shared-prep criteria. Any remaining portability work is provider-specific.
