...
This document describes the current implementation in `src/artifacts/service.ts`.
...
- `buildish-mammoth-cache-gradle-delta-<sanitized-job-name>-run-<runId>-attempt-<runAttempt>-<cacheKeyHash>-<deltaManifestHash>`
...
This avoids leaking worker-local filesystem paths into exchanged artifacts while preserving the relative-path and snapshot information needed for later merge/apply steps.
