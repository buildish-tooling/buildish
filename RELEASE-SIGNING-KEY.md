...
Update [`site/static/KEYS`](site/static/KEYS):

1. Preserve its license preamble.
2. Remove the placeholder saying that no signing keys are published.
3. Add comments containing the primary fingerprint, signing-subkey
   fingerprint, and signing-subkey expiration date.
4. Append the complete contents of `buildish-public.asc`.
...
- Update the human-readable fingerprint, status, and expiration comments at the
  same time. Treat those comments as documentation; the armored certificate is
  the authoritative cryptographic data.
