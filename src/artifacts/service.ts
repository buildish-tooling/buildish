...
const DELTA_ARTIFACT_NAME_PREFIX = 'cache-gradle-delta';
...
export interface DeltaArtifactPackageMetadata {
  ...
  readonly artifactType: 'cache-gradle-delta';
...
export async function stageDeltaArtifactPackage(
  ...
  const stagingDirectory = await mkdtemp(path.join(stagingParent, 'cache-gradle-delta-artifact-'));
  ...
  const metadata: DeltaArtifactPackageMetadata = {
    ...
    artifactType: 'cache-gradle-delta',
    artifactName,
...
export async function downloadAndVerifyDeltaArtifactPackage(
  ...
  const downloadDirectory = await mkdtemp(
    path.join(parentDirectory, 'cache-gradle-delta-download-'),
  );
...
function validateArtifactType(value: unknown): 'cache-gradle-delta' {
  if (value !== 'cache-gradle-delta') {
    throw new Error(`Delta artifact metadata type must be 'cache-gradle-delta'.`);
  }

  return 'cache-gradle-delta';
}
