...
from pipeline.tests.test_support import BUILDISH_DISCLAIMER, TestCaseHelpers, text_block, write_files, write_text
...
class SitePipelineTest(TestCaseHelpers, unittest.TestCase):
    ...
    @staticmethod
    def catalog_payload(*components: dict[str, object], defaults: dict[str, object] | None = None) -> dict[str, object]:
        payload: dict[str, object] = {"schemaVersion": 1, "components": list(components)}
    ...
    def test_staged_section_to_yaml_data_omits_null_optional_fields(self) -> None:
        self.assertEqual(
            ...
            VersionDescriptor(kind="unreleased", label="Preview", path="/components/foo/unreleased/").to_yaml_data(),
        )
        self.assertEqual(
            {"label": "Preview", "path": "/components/foo/unreleased/"},
            LifecycleUnreleased(label="Preview", path="/components/foo/unreleased/").to_yaml_data(),
        )
        self.assertEqual({"version": "v1.2.3"}, LifecycleLatestStable(version="v1.2.3").to_yaml_data())

    def test_markdown_front_matter_serializes_typed_component_payload_models(self) -> None:
    ...
    def test_components_catalog_from_yaml_path_returns_typed_models(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            catalog = ComponentsCatalog.from_yaml_path(repo_root / "site" / "components.yaml")
    ...
    def test_components_catalog_from_yaml_path_accepts_null_weight(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                repo_root / "site" / "components.yaml",
                self.catalog_payload({"slug": "mammoth-cache", "localDir": "buildish-mammoth-cache", "weight": None}),
            )
    ...
    def test_components_catalog_from_yaml_path_rejects_null_defaults_and_components(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir) / "buildish"
            (repo_root / "site").mkdir(parents=True)
            self.write_yaml(repo_root / "site" / "components.yaml", {"schemaVersion": 1, "defaults": None, "components": None})
    ...
    def test_load_component_metadata_rejects_invalid_tag_pattern(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir) / "buildish-mammoth-cache"
            (repo_root / "site").mkdir(parents=True)
            self.write_yaml(repo_root / "site" / "component.yaml", {"schemaVersion": 1, "versioning": {"tagPattern": "["}})
    ...
    def test_load_component_metadata_rejects_invalid_release_line_aliases(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                ...
                {
                    "schemaVersion": 1,
                    "lifecycle": {"releaseLines": [{"line": "v1", "latest": "v1.2.3", "status": "maintained", "aliases": "stable"}]},
                },
            )
    ...
    def test_load_component_metadata_rejects_null_nested_sections(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                repo_root / "site" / "component.yaml",
                {"schemaVersion": 1, "component": None, "content": None, "versioning": None, "lifecycle": None, "navigation": None},
            )
    ...
    def test_build_falls_back_to_default_docs_root_when_component_value_is_null(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                repo_root / "site" / "components.yaml",
                self.catalog_payload(
                    {"slug": "mammoth-cache", "localDir": "buildish-mammoth-cache", "docsRoot": None},
    ...
    def test_build_stages_docs_and_lifecycle_from_component_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo_root.mkdir()
            (repo_root / "site").mkdir()
            write_text(repo_root / "DISCLAIMER", BUILDISH_DISCLAIMER)
            catalog = {
                "schemaVersion": 1,
                "defaults": {
                    "metadataFile": "site/component.yaml",
            ...
            self.write_yaml(
                ...
                {
                    "schemaVersion": 1,
                    "component": {"slug": "site", "displayName": "Site", "repository": "https://github.com/apache/buildish"},
            ...
            self.assert_paths_exist(
                ...
                repo_root / "site" / ".stage" / "content" / "components" / "mammoth-cache" / "unreleased" / "docs" / "wrapper-provisioning.md",
                repo_root / "site" / ".stage" / "static" / "components" / "mammoth-cache" / "unreleased" / "assets" / "images" / "logo.svg",
                repo_root / "site" / ".stage" / "content" / "components" / "no-gradle-wrapper-jar" / "unreleased" / "_index.md",
                repo_root / "site" / ".stage" / "content" / "components" / "site" / "unreleased" / "docs" / "_index.md",
            ...
            self.assert_not_contains_any(staged_root_index, "redirect_url:", "## Incubation status")
            ...
            self.assert_contains_all(
                ...
                "Apache Buildish (Incubating) is an effort undergoing incubation",
            )
            self.assert_contains_all(
                root_front_matter["incubator_disclaimer_paragraphs"][2],
            ...
            self.assert_contains_all(components_index, "title: Components", "type: docs", "Browse staged components")
            ...
            self.assertIn("Helper scripts for Gradle wrapper usage.", no_wrapper_component_index)
            self.assertIn("keeps wrapper bootstrapping lean", no_wrapper_component_index)

            site_component_index = (repo_root / "site" / ".stage" / "content" / "components" / "site" / "_index.md").read_text(encoding="utf-8")
            self.assertIn("title: Buildish Site", site_component_index)
            self.assertIn("weight: 100", site_component_index)
            self.assertIn("Buildish site publishing and shared documentation tooling.", site_component_index)
            self.assertNotIn("Site implementation and infrastructure docs.", site_component_index)

            site_unreleased_index = (repo_root / "site" / ".stage" / "content" / "components" / "site" / "unreleased" / "_index.md").read_text(encoding="utf-8")
            self.assertIn("## Docs", site_unreleased_index)
            self.assertIn("[Apache Buildish Site Documentation](/components/site/unreleased/docs/)", site_unreleased_index)
            self.assertNotIn("Open staged assets", site_unreleased_index)

            site_version_document = ComponentVersionDocument.from_yaml_path(
                repo_root / "site" / ".stage" / "content" / "components" / "site" / "unreleased" / "version.yaml"
            )
            site_version_metadata = (repo_root / "site" / ".stage" / "content" / "components" / "site" / "unreleased" / "version.yaml").read_text(encoding="utf-8")
            self.assertIn("docsPath: /components/site/unreleased/docs/", site_version_metadata)
    ...
    def test_rejects_metadata_path_escape(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                mammoth / "site" / "component.yaml",
                {"schemaVersion": 1, "component": {"displayName": "Bad metadata"}, "content": {"docsRoot": "../escape"}},
            )
    ...
    def test_rejects_component_escape_outside_workspace_parent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                repo_root / "site" / "components.yaml",
                self.catalog_payload(
                    {"slug": "bad-component", "displayName": "Bad", "localDir": "../escape"},
                    defaults={"metadataFile": "site/component.yaml", "pagesRoot": "site/pages", "docsRoot": "site/docs"},
                ),
            )
    ...
    def test_rejects_assets_path_escape(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_yaml(
                mammoth / "site" / "component.yaml",
                {"schemaVersion": 1, "component": {"displayName": "Bad assets metadata"}, "content": {"assetsRoot": "../escape"}},
            )
    ...
    def test_build_stages_vendor_assets_when_available(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assert_paths_exist(
                ...
                repo_root / "site" / ".stage" / "static" / "js" / "vendor" / "mermaid.min.js",
                repo_root / "site" / ".stage" / "static" / "js" / "vendor" / "lunr.min.js",
            )
...
"""
    ...
    def test_build_can_skip_preview_generation_in_watch_mode(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo_root.mkdir()
            (repo_root / "site").mkdir(parents=True)
            write_text(repo_root / "DISCLAIMER", "Buildish disclaimer.\n")
            self.write_yaml(
                repo_root / "site" / "components.yaml",
                self.catalog_payload(
                    {"slug": "mammoth-cache", "localDir": "buildish-mammoth-cache"},
    ...
    def test_collect_watch_roots_includes_catalog_component_inputs_and_missing_repo_parent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            repo_root = workspace / "buildish"
            repo_root.mkdir()
