...
Version binding: this document is versioned alongside the repository. A report
is evaluated against the model in the affected commit or release tag, not
necessarily against `main`. (documented)
...
Status: development review aid; not yet maintainer-ratified or canonical. It is
not an authoritative severity, advisory, or CVE decision. Assumptions marked
`(inferred)` and the proposed answers in Section 14 remain subject to maintainer
...
Confidence: the model distinguishes documented behavior from inferred
assumptions so reviewers can identify claims that still need explicit project
policy or implementation evidence.
...
## 14. Maintainer review backlog

The following questions identify assumptions that should be made explicit as
the implementation evolves. They do not override the documented trust
boundaries or decide whether a private report is a vulnerability.

Wave 1: scope and boundaries.

- Should this Website-component document remain the canonical threat model for
  the aggregate repository, while Site Pipeline keeps its separate model? Lands
...
No `threat-model.yaml` sidecar is currently included. This prose document is
the current development review artifact, not a ratified canonical policy. If
the project later wants automated or AI-assisted triage, first complete the
maintainer review and then derive a sidecar containing entry-point trust
levels, component scope, configuration variants, claimed and disclaimed
properties, known non-findings, and disposition labels from the reviewed
document. (inferred)
