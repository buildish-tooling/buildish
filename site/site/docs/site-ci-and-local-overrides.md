...
- how the site build should resolve project inputs in CI, and
...
1. one Git repository provides more than one logical site project,
2. a developer has a non-standard checkout layout that should not be committed to the shared catalog.

## Proposed model

The catalog should separate project identity from workspace discovery.

### Project identity in the shared catalog

The shared catalog should describe the **canonical source** of each site project:

- `slug` — stable public project identifier,
- `repository` — canonical Git repository URL,
- `sourceRoot` — path inside that repository that owns the project's site contract,
- `metadataFile`, `docsRoot`, `assetsRoot` — paths relative to `sourceRoot`.

`sourceRoot` defaults to `.`. This makes it possible for one repository to host multiple site projects.
...
Developers should provide local path overrides in an ignored file such as `site/projects.local.yaml`.
...
- or the set of projects that the published site claims to contain.
...
CI should not rely on local-style path guessing. Instead, it should produce a resolved build manifest early in the workflow that records, for each included project/version input:
...
### One repository can host multiple site projects
...
That means the site build can model a mono-repo or an internal project like the site itself without pretending that every project needs its own repository.

### Local overrides become machine-local only

Developers can map project slugs to actual checkout roots without committing those paths into `site/projects.yaml`.
...
2. local overrides from `site/projects.local.yaml`, when present
3. catalog checkout hints or conventional sibling discovery
4. missing checkout → skip project cleanly for local builds, fail only if CI marks it required

Content structure resolution should remain separate:

1. per-project overrides in `site/projects.yaml`
2. sub-project metadata
3. catalog defaults
4. built-in defaults
...
```mermaid
flowchart TD
    A[load site/projects.yaml] --> B[resolve project repositories]
...
## Scaling with many projects and many tags

The expensive part is not the unreleased build; it is release aggregation if every site build re-checks every historical tag for every project.
...
- `P` projects,
- `T` historical exact-release tags per project,

then a naive "clone and stage every tag on every build" model behaves roughly like **O(P × T)** content materializations. That becomes too expensive quickly once the project count and release count both grow.
...
- **unreleased docs**: resolve and stage from a live checkout of each project's default branch for the current run,
...
For each project, the resolver should:
...
- Repositories that back multiple site projects should be checked out once and reused.
...
This makes the operational cost closer to "number of repositories involved in this build" than "number of projects times number of versions".
...
2. optionally clone any sub-project repositories of interest,
3. create `site/projects.local.yaml` if the checkout layout is non-standard,
4. run `make -C site serve` or `make -C site build`.

Developers should not need every repository to be present. Missing projects should continue to degrade cleanly in local builds.
...
1. Add `sourceRoot` to the shared catalog and treat it as the logical project root inside a repository.
2. Introduce `site/projects.local.yaml` as an ignored local path-binding file.
