...
- If you are not sure where something belongs, email the [development contact](mailto:dev@buildish.org).
...
For the aggregate Buildish repository, run the canonical gate from the
repository root:

- `make check`
- `make rat-check` when adding files or changing legal material

For site-only iteration, `cd site` before running targets such as `make test`,
`make integration-test`, or `make build-local`.
